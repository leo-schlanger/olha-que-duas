...
import { ArrowLeft, Camera, ImageIcon, Loader2, AlertCircle } from 'lucide-react';
import { Button } from '@/components/ui/button';
import Header from '@/components/Header';
import Footer from '@/components/Footer';
import BackToTop from '@/components/BackToTop';
import { Animated, AnimatedCounter } from '@/components/ui/animated';
import { GalleryTimeline } from '@/components/gallery';
...
export default function Gallery() {
  const { data: albumsByYear, isLoading, error } = useGalleryAlbums();

  // Calculate totals
  const totalAlbums = albumsByYear?.reduce((acc, year) => acc + year.albums.length, 0) || 0;
  const totalPhotos = albumsByYear?.reduce((acc, year) =>
    acc + year.albums.reduce((albumAcc, album) => albumAcc + album.photo_count, 0)
  , 0) || 0;

  // SEO Meta Tags
  useMetaTags({
    title: 'Galeria',
    description: 'Explore a nossa galeria de fotos de entrevistas, eventos e bastidores do Olha que Duas. Momentos especiais capturados em imagens.',
    url: 'https://www.olhaqueduas.com/galeria',
    jsonLd: getPageBreadcrumbJsonLd('Galeria', 'https://www.olhaqueduas.com/galeria'),
  });
      ...
      <main className="flex-1 pt-24 md:pt-28">
            ...
            <div className="max-w-4xl mx-auto text-center">
              ...
              <Animated animation="fade-down">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-vermelho/10 border border-vermelho/20 mb-6">
                  <Camera className="w-4 h-4 text-vermelho" />
              ...
              <Animated animation="fade-up" delay={100}>
                <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-display font-bold text-cream mb-6 leading-tight">
                  Momentos{' '}
                  <span className="text-gradient-brand">Especiais</span>
                </h1>
              </Animated>
              ...
              <Animated animation="fade-up" delay={200}>
                <p className="text-lg md:text-xl text-cream/70 max-w-2xl mx-auto leading-relaxed mb-10">
                  Explore a nossa coleção de fotos de entrevistas, eventos e bastidores.
                  ...
                  <div className="flex justify-center gap-8 md:gap-16">
                    <div className="text-center">
                      <div className="text-3xl md:text-4xl font-display font-bold text-vermelho">
                        <AnimatedCounter value={totalAlbums} />
                      </div>
                    ...
                    <div className="text-center">
                      <div className="text-3xl md:text-4xl font-display font-bold text-cream">
                        <AnimatedCounter value={totalPhotos} suffix="+" />
                      </div>
                      <div className="text-xs text-cream/50 uppercase tracking-wider mt-1 flex items-center gap-1 justify-center">
        ...
        <section className="py-12 md:py-16">
          <div className="container mx-auto px-4 sm:px-6">
            ...
            {/* Timeline */}
            {!isLoading && !error && albumsByYear && (
              <GalleryTimeline albumsByYear={albumsByYear} />
            )}

            {/* Empty state */}
            {!isLoading && !error && albumsByYear?.length === 0 && (
              <Animated animation="fade-up" className="text-center py-16">
                <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-muted/50 flex items-center justify-center">
                ...
                <p className="text-muted-foreground">
                  Em breve teremos fotos para partilhar.
                </p>
              </Animated>
            )}
          </div>
